		...
		<Card>
			<CardHeader className="flex justify-between items-start gap-5">
				<div className="flex flex-col justify-center text-left items-start">
					<CardTitle className="font-semibold text-xl text-cBlack">
						Номер заказа:
					</CardTitle>
					<CardDescription className="font-medium text-cLightBlue text-base">
						{item.orderNumber}
					</CardDescription>
				</div>
				<div className="flex flex-col justify-center text-left items-start">
			...
			<CardContent className="flex flex-col gap-5">
				{item.list.map((item) => (
					<HistoryViewBlock key={item.id} item={item} />
				))}
			</CardContent>
		</Card>
	);
