	...
	const downloadPDF = async () => {
		try {
			const response = await fetch(
				`${import.meta.env.VITE_API_URL}/orders/${orderId}/pdf`,
				{
					headers: {
			...
			{loading ? (
				...
				<div className="space-y-5 relative">
					...
					<Card className="flex flex-col items-start gap-5 p-5">
						<CardTitle className="text-xl font-semibold text-black/80">
							Заказ № {id}
						</CardTitle>
						<div className="w-full flex justify-between items-center gap-5">
							<div className="w-full space-y-2">
								...
								<div className="flex justify-between items-center text-black/80">
									...
									<span className="text-base font-semibold">
										{formatPrice(
											order?.client?.balance as number
										)}
									</span>
								</div>
							</div>
						...
						<Table className="border-spacing-y-2 border-separate">
							<TableHeader>
								<TableRow className="border-none hover:bg-white">
									<TableHead className="text-left text-base font-semibold">
										#
									</TableHead>
									<TableHead className="text-left text-base font-semibold">
										Наименование
											...
											<TableCell className="text-base text-left relative after:content-[''] after:absolute after:right-0 after:top-0 after:h-full after:w-[1px] after:bg-[#CBCBCB]/50">
												<div className="flex justify-start items-center gap-1">
													<img
														...
														alt="product image"
														className="w-10 h-10 bg-white rounded-md border object-cover border-cGray"
													/>
													{item?.product?.name}
												</div>
											</TableCell>
